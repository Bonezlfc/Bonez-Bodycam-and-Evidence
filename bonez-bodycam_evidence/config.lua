...
-- ── Night Shifts permissions (validated server-side only) ─────────────────
-- Rank permission string required to view evidence in the hub.
-- Falls back to police-on-shift check if the rank has no permissions array.
Config.EvidenceViewPermission = 'evidence_view'

-- Night Shifts adminLevel required to delete clips.
-- txAdmin / server ACE 'command' permission always grants admin access regardless.
Config.EvidenceAdminLevel = 2

-- Night Shifts department IDs that count as "police".
-- Must match the IDs returned by exports['night_shifts_mdt']:GetDepartments().
-- Keep this list in sync with Config.PoliceDepartmentIds in bonez-bodycam/config.lua.
